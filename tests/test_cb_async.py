...
# 模拟的异步函数
async def construct_three() -> int:
    await asyncio.sleep(1)  # 模拟异步操作
    return 3


async def construct_four() -> int:
    await asyncio.sleep(1)  # 模拟异步操作
    return 4
...
class TestAsyncCBV:
    ...
    def test_response_models(self, router: APIRouter) -> None:
        ...
        class CBV:
            ...
            @classmethod
            async def __async_init__(cls) -> "CBV":
                c = cls()
                c.three = await construct_three()
                c.four = await construct_four()
                return c
            ...
            @router.get("/asum", response_model = int)
            async def async_sum(self) -> int:
        ...
        response_async = client.get("/asum")
        assert response_async.status_code == 200
        assert response_async.content == b"7"
